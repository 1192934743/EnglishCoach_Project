...
class _ChatScreenState extends ConsumerState<ChatScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          tr(ref, 'Simulation Practice', '场景模拟对练'),
        ...
        actions: [
          IconButton(
            ...
            tooltip: tr(ref, "Toggle Chat History", "切换历史记录"),
      ...
      body: SafeArea(
        ...
        child: Stack(
          children: [
            ...
            // ================= 2. 主体自适应排版 (去除所有导致挤压的过渡动画) =================
            Column(
              children: [
                // 🌟 修复：不再使用 AnimatedContainer，直接根据状态瞬间锁定精确的边距和高度空间
                Container(
  ...
  Widget _buildEmptyState(bool isListening, double fontSize, WidgetRef ref) {
    return Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          ...
          Text(
            isListening
                ...
                : tr(ref, "Tap the pill below to start!", "点击下方开始对话！"),
  ...
  // 🌟 修复：直接计算精确的包围盒大小，拒绝布局挤压
  Widget _buildAvatar(bool isLis, bool isSpe, double fs) {
    // 根据状态瞬间锁定精确的像素尺寸
    ...
    return AnimatedBuilder(
      animation: _glowAnimation,
      builder: (context, _) {
        // 呼吸效果只作用于内部元素的缩放 (Transform.scale)，这不会引发外部容器的重新布局！
        ...
        return Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            // 🌟 使用定死的 SizedBox，锁死图片需要的绝对空间，拒绝让它霸占多余区域
            SizedBox(
              width: exactSize,
              height: exactSize,
              child: Stack(
                ...
                children: [
                  if (isLis || isSpe)
                    Transform.scale(
                      scale: visualScale * 1.15, // 呼吸光圈略大于原图
                      child: ImageFiltered(
            ...
            // 状态指示文字，瞬间隐藏/显示，不带挤压动画
            if (!_showHistory) ...[
              const SizedBox(height: 16),
              Text(
                isLis
                    ? tr(ref, "Listening...", "教练正在倾听...")
                    : (isSpe
                          ? tr(ref, "Speaking...", "教练回复中...")
                          : tr(ref, "Ready", "准备就绪")),
                style: TextStyle(
                  fontSize: fs + 2,
                  fontWeight: FontWeight.bold,
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              Text(
                                tr(ref, "You can reply:", "推荐回复 (点击朗读):"),
                                style: const TextStyle(
                                  fontWeight: FontWeight.bold,
                                  color: Colors.grey,
                                  fontSize: 13,
