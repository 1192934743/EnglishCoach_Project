...
REST 端点：
- GET /api/topics — 话题列表（附带用户掌握度）
- GET /api/stats  — 用户学习统计
"""
...
def _get_topics_for_user(user_id: Optional[str]) -> list[dict]:
    with _db() as db:
...
def _get_stats_for_user(user_id: str) -> dict:
    import datetime
...
async def get_topics(user_id: Optional[str] = Query(default=None)):
    ...
    """
    data = await run_in_threadpool(_get_topics_for_user, user_id)
...
async def get_stats(user_id: str = Query(...)):
    ...
    返回用户学习统计数据。
    """
    data = await run_in_threadpool(_get_stats_for_user, user_id)
