...
load_dotenv("config.env")  # 明确加载 config.env
...
async def websocket_endpoint(websocket: WebSocket, user_id: str = None):
    ...
    db = SessionLocal()
    current_user = None
    current_topic = db.query(Topic).first()  # 简化获取，或根据 ACTIVE_SCENE 查询
        ...
        while True:
            ...
            if "bytes" in message:
                ...
                if action == "swap_role":
                    ...
                    async with ws_lock: await websocket.send_text(
                        json.dumps({"event": "role_swapped", "is_flipped": is_flipped}))
                    continue
                ...
                if action == "ping":
                    ...
                    if not user_text:
                        if not is_test_mode:
                            async with ws_lock: await websocket.send_text(json.dumps({"event": "tts_finished"}))
                        continue

                    session_ctx["phase_turns"] += 1
                    logger.info(f"✅ 回合 {session_ctx['phase_turns']} ({session_ctx['phase']}) | 你说: {user_text}")

                    if current_user and current_topic:
                        await evaluate_and_check_progress(db, current_user.id, current_topic.id, user_text,
                                                          session_hits, session_ctx, websocket, ws_lock)

                        if advance_state_machine(session_ctx, db, current_topic, session_hits):
                            chat_history[0]["content"] = build_dynamic_prompt(current_user, is_flipped, session_ctx)
                    ...
                    if len(chat_history) > 11: chat_history = [chat_history[0]] + chat_history[-10:]
