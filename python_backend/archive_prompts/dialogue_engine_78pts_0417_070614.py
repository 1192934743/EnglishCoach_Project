...
def build_dynamic_prompt(user: User, is_flipped: bool, session_ctx: dict):
    ...
    # 🌟 核心防呆结构：全部使用数组 append，禁止使用多行字符串！
    prompt_blocks = []
    ...
    prompt_blocks.append("1. OPEN-ENDED: NEVER ask simple Yes/No questions. Always use What/How/Why/Which to force longer user responses.")
    prompt_blocks.append("2. AGGRESSIVE PIVOT: If the user drifts off-topic, give a 1-sentence friendly reply, then IMMEDIATELY return to the current phase's goal.")
    prompt_blocks.append("3. CONVERSATIONAL: Keep replies engaging but concise (15 to 30 words).")
    prompt_blocks.append("4. PLAIN TEXT: Reply in plain English text only. Spell out numbers/symbols.")
    prompt_blocks.append("5. NO SPOILERS: Never explicitly list the secret target words to the user.")
    prompt_blocks.append("")
    ...
    if phase == "ICE_BREAKING":
        ...
        prompt_blocks.append(f"Goal: Chat naturally to build rapport in this {scene_name} context.")
        prompt_blocks.append("STRICT FIREWALL: DO NOT process the main transaction or mission in this phase! DO NOT execute the core task yet!")
        ...
        prompt_blocks.append(f"Goal: Now officially execute the main transaction/mission of this {scene_name}.")
        prompt_blocks.append(f"SECRET MISSION: Naturally guide the user to say these NEW words: {new_targets}.")
        if history_instruction:
            prompt_blocks.append(history_instruction)
        prompt_blocks.append("ADVANCE RULE: Output [ADVANCE] ONLY when the core task is fully complete (e.g., final results given, ready to conclude or pay).")
    
    elif phase == "EVENT_EXTENSION":
        prompt_blocks.append("PHASE 3: EVENT EXTENSION (The Twist)")
        prompt_blocks.append("Goal: Introduce a sudden complication or unexpected twist.")
        prompt_blocks.append(f"SYSTEM OVERRIDE: Introduce this specific scenario: '{session_ctx.get('current_event', '')}'")
        prompt_blocks.append("ADVANCE RULE: Fully embrace this twist. Once the user successfully responds to and resolves this twist, output [ADVANCE].")
    
    elif phase == "WRAP_UP":
        prompt_blocks.append("PHASE 4: WRAP UP (Conclusion)")
        prompt_blocks.append("Goal: End the current interaction naturally (e.g., finalize process, saying goodbye).")
        prompt_blocks.append("ADVANCE RULE: Conclude the conversation gracefully. Output [ADVANCE] in your final goodbye message to reset the simulation for the next round.")
